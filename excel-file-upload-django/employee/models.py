...
class UserManager(models.Manager):
    def register_validator(self, postData):
        ...
        # Check in the db that, whether the incoming email is previosuly stored in the db or not
    ...
    def login_validator(self, postData):
        ...
        if len(postData['password']) < 1:
            ...
            print(user)
            # if not crypt.checkpw(postData['password'].encode(), user.password.encode()):
            if postData['password']!=user.password:
            # not crypt.checkpw(postData['password'].encode(), user.password.encode()):
